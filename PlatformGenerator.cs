   ...
   // public GameObject[] thePlatforms;
	...
	void Update () {
        ...
        if (transform.position.x < generationPoint.position.x)
        {
            ...
            if (num <= 30)
			{
				float size = platformWidths[platformSelector];

				Vector2 newPos = new Vector2(newPlatform1.transform.position.x - size, newPlatform1.transform.position.y + 4f);	//new vector2 with -size of object
				...
				if(platformSelector == 2)	//orbs with crystal - 10 orbs for bridge
				{
					...
					theCrystalGenerator.SpawnCrystal(new Vector2(newPlatform1.transform.position.x + size + 1f, newPlatform1.transform.position.y + 5f));

				}
				else if(platformSelector == 1)
				{
					theOrbGenerator.SpawnOrbs(newPos, 3);//new Vector2(newPlatform1.transform.position.x - size, newPlatform1.transform.position.y + 3f), 10);

					theCrystalGenerator.SpawnCrystal(new Vector2(newPlatform1.transform.position.x + size + 1f, newPlatform1.transform.position.y + 5f));

				} */
				theOrbGenerator.SpawnOrbs(newPos, 2);//new Vector2(newPlatform1.transform.position.x - size, newPlatform1.transform.position.y + 3f), 10);
			...
			else if ((num >= 31 && num <=45) && (thePlayer.transform.position.x > 500))
            {
				...
				float size = platformWidths[platformSelector3];
				...
				theOrbGenerator.SpawnOrbs(newPos, 4);//new Vector2(newPlatform1.transform.position.x - size, newPlatform1.transform.position.y + 3f), 10);
